...
def load_model_and_tokenizer():
    # HF’dan indir (örneğin ~/.cache/huggingface/...)
    local_dir = snapshot_download("ackermanBaki/llada-turkish")

    # tokenizer.json’ı yükle
    tok_path = os.path.join(local_dir, "tokenizer.json")
    tokenizer = Tokenizer.from_file(tok_path)

    # config’u kendi sınıfınızla oluşturun
    config = LLaDAConfig(vocab_size=tokenizer.get_vocab_size())

    # model ağırlıklarını manuel yükleyin
